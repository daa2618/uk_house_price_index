...
class FileVersion:
    ...
    def get_all_files(self) -> list[Path]:
        ...
        if self.folder_exists():
            file_paths = [
                ...
                if file_path.is_file() and self.file_name in file_path.name
            ]
            return file_paths
    ...
    def _search_for_dates(self) -> list[str] | None:
    ...
    def check_version(self) -> Any:
        ...
        if self.folder_exists:
            ...
            all_files = [
                f"{self.file_name}{datetime.datetime.strftime(x, self.date_fmt)}{self.extension}" for x in dates
            ]
    ...
    @property
    def _latest_file_path(self) -> Path | None:
        """
        ...
        latest_file = [
            file_path
            for file_path in self.base_path.iterdir()
            if file_path.is_file() and pattern.findall(file_path.name)
        ]
        if latest_file:
    ...
    def _fetch_dates_from_file_names(self) -> list[datetime.datetime] | None:
        if not self.file_name.endswith("_"):
            self.file_name = f"{self.file_name}_"
