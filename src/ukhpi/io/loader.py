...
from ukhpi.io.response import Response
...
class Dataset:
    ...
    def __init__(self, debug: bool = False, **kwargs):
        ...
        self._supported_extensions = ["csv", "ods", "xlsx", "xls", "json", "pdf", "text/csv", "geojson"]
        self.debug = debug
    ...
    def _check_extension(self, extension: str):
        ...
        if not any(extension.endswith(x) for x in self._supported_extensions) and not any(
            x in re.sub("\\.", "", extension) for x in self._supported_extensions
        ):
        ...
        if self.doc_url:
            try:
                ...
                try:
                    ...
                    if "github" in self.doc_url:
                        doc_url = re.sub(
                            "blob", "refs/heads", urlparser.urljoin("https://raw.githubusercontent.com", path)
                        )
    ...
    def _load_for_extension(self):
        ...
        if extension:
            ...
            elif extension.endswith("xlsx") or extension.endswith("xls") or "xlsx" in extension or "xls" in extension:
                try:
            ...
            elif extension.endswith("geojson") or "geojson" in extension or "geo+json" in extension:
                try:
...
class PostProcess:
    @staticmethod
    def set_columns_from_index_and_drop_rows(df: pd.DataFrame, col_index: str | int) -> pd.DataFrame | None:
    ...
    @staticmethod
    def convert_data_types_of_cols(df: pd.DataFrame, d_type: str) -> pd.DataFrame | None:
