...
class PricePaidData:
    ...
    def clean_df(self) -> pd.DataFrame:
        ...
        if self._cleaned_df is None or self._cleaned_df.empty:
            ...
            results_df = results_df.assign(
                address=[
                    f"{paon}, {street}, {postcode}"
                    for paon, street, postcode in results_df[["paon", "street", "postcode"]].values
                ]
            )
    ...
    def calculate_appreciated_prices(self) -> pd.DataFrame:
        ...
        for address in results_df["address"].unique():
            filtered_for_address = results_df.loc[
        ...
        return pd.DataFrame(out).sort_values("appreciation_per_year", ascending=False).reset_index(drop=True)
