...
class InitLogger:
    ...
    def _create_timed_rotating_file_handler(
        self, log_file_path: str, when: str = "midnight", interval: int = 1, backup_count: int = 7, verbose: bool = True
        ...
        handler = logging.handlers.TimedRotatingFileHandler(
            filename=log_file_path, when=when, interval=interval, backupCount=backup_count
        )
        _print(f"TimedRotatingFileHandler initialized for {log_file_path}.\n", verbose)
        return handler

    def _create_file_handler(
        self, log_file_path: str, write_mode: str = "a", verbose: bool = True
    ) -> logging.FileHandler:
    ...
    def _set_handler_formatter(
        self, handler: logging.Handler, log_format: str, date_format: str, verbose: bool = True
    ) -> None:
    ...
    def _add_handler_to_logger(self, logger: logging.Logger, handler: logging.Handler, verbose: bool = True) -> None:
        ...
        if isinstance(
        ...
        ):  # Checks FileHandler and its subclasses like TimedRotatingFileHandler
            for h in logger.handlers:
                if isinstance(h, type(handler)) and getattr(h, "baseFilename", None) == handler.baseFilename:
                    is_duplicate = True
                    _print(
                        f"Handler {type(handler).__name__} for file '{handler.baseFilename}' seems to be a duplicate. Skipping add.",
                        verbose,
                    )
                    break
            ...
            for h in logger.handlers:  # Check stream for StreamHandlers (e.g., sys.stdout, sys.stderr)
                if isinstance(h, type(handler)) and getattr(h, "stream", None) == getattr(handler, "stream", None):
                    is_duplicate = True
                    _print(
                        f"Handler {type(handler).__name__} for stream '{getattr(handler, 'stream', 'N/A')}' seems to be a duplicate. Skipping add.",
                        verbose,
                    )
                    break
...
class AppLogger(InitLogger):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if clear_existing_handlers and self.logger.hasHandlers():
            _print(
                f"Logger '{self.logger.name}' has existing handlers. Clearing them as per configuration.", self.verbose
            )
        ...
        if log_directory and actual_log_file_name:
            try:
                self.log_dir_path = _checkDirectory(
                    log_directory, pardir=kwargs.get("pardir_for_log_dir")
                )  # Allow pardir override

                full_log_file_path = os.path.join(
                    self.log_dir_path, f"{actual_log_file_name}{actual_log_file_extension}"
                )
                ...
                if use_timed_rotating_handler:
                    ...
                    self.file_handler = self._create_file_handler(
                        log_file_path=full_log_file_path, write_mode=file_write_mode, verbose=self.verbose
                    )

                self._set_handler_level(
                    self.file_handler, self.log_level, verbose=self.verbose
                )  # File handler typically uses logger's level
                ...
                _print(
                    f"Error setting up file handler for logger '{self.logger_name}': {e}", verbose=True
                )  # Always print this error
            except Exception as e:
                _print(f"Unexpected error setting up file handler for '{self.logger_name}': {e}", verbose=True)
        else:
            _print(
                f"File logging skipped for '{self.logger_name}' (log_directory or log_file_name not provided).",
                self.verbose,
            )
    ...
    def exception(self, message: str, *args, **kwargs):
        self.logger.exception(message, *args, **kwargs)  # Logs ERROR with exc_info=True
...
class BasicLogger(AppLogger):
    ...
    def __init__(
        ...
        log_file_extension: str = ".log",
        **kwargs,
    ):  # Pass through any other AppLogger params
        super().__init__(
