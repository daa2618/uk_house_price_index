import pandas as pd  # noqa: F401 — re-exported via `from ukhpi.plotting.theme import *`
...
class SubplotSpecs:
    ...
    def get_subplot_matrices(self):
        ...
        rows = {x: y for x, y in (zip(range(1, n + 1), rows, strict=False))}
        cols = {x: y for x, y in (zip(range(1, n + 1), [1, 2, 3] * (n), strict=False))}
        return rows, cols

    def get_subplot_specs(self, d=None):
