...
from ukhpi.plotting.theme import (  # re-exports used by ukhpi.plotting.hpi_plots
    SubplotSpecs,  # noqa: F401
    go,
    make_subplots,
    pd,
    px,
...
class PostProcess:
    def make_number_readable(num: (int | float)) -> str:
        ...
        formatted = (
            f"{sign * abs_num:.2f}".rstrip("0").rstrip(".")
            if "." in f"{sign * abs_num:.2f}"
            else f"{sign * abs_num:.0f}"
        )
...
class BasicPlots:
    ...
    def _adjust_text_formatting(
        ...
        for trace in fig.data:
            if trace.type == "bar":
                ...
                trace.textfont = dict(weight="bold", family="sans serif", color=text_color)
        ...
        fig.update_layout(uniformtext=dict(minsize=text_size, mode="hide"))
    ...
    def plot_null_and_non_null_counts(self, show_as_percentage: bool = False) -> go.Figure:
        ...
        for col in self.df.columns:
            ...
            fig.add_trace(
                go.Bar(x=[col], y=[na], name="Null", meta="Null", marker_color="Aquamarine", showlegend=False)
            )

            fig.add_trace(
                go.Bar(
                    x=[col], y=[nonNa], name="Non-null", meta="Non-null", marker_color="LightSeaGreen", showlegend=False
                )
            )
        ...
        if show_as_percentage:
            fig.update_layout(
                barnorm="percent", title=dict(text="Percentage of null and non-null values by column", x=0.5)
            )
            fig.update_yaxes(title="Percentage")
            fig.update_traces(
                hovertemplate="<b>Column : </b>%{x}<br>" + "<b>%{meta} : </b>%{y:.2f}%<br>" + "<extra></extra>"
            )
        else:
            fig.update_layout(title=dict(text="Count of null and non-null values by column", x=0.5))
            fig.update_yaxes(title="Count")
            fig.update_traces(
                hovertemplate="<b>Column : </b>%{x}<br>" + "<b>%{meta} : </b>%{y}<br>" + "<extra></extra>"
            )
        return fig
    ...
    def plot_histogram_of_column(self, column: str, show_labels: bool = False, orientation: str = "v") -> go.Figure:
        ...
        if show_labels:
            fig.update_traces(text_template="%{y:,}", textfont_size=20)

        fig.update_layout(
            # bargap=0.1,
            title=dict(text=f"<b>Histogram of {column}</b>", x=0.5),
        )
    ...
    def plot_histogram_of_data_frame(self):
        ...
        bar_colors = px.colors.sample_colorscale(px.colors.sequential.Burgyl, len(array))
        fig = make_subplots(
            rows=n_rows,
            cols=n_cols,
        ...
        for a, column in enumerate(array):
            ...
            fig.add_trace(data, row=rows[a + 1], col=cols[a + 1])
        fig.update_layout(height=1000, title=dict(text="<b>Histogram of Columns<b>", x=0.5))

        fig.update_xaxes(tickangle=45, nticks=12)

        fig.update_yaxes(nticks=2)

        fig.update_traces(showlegend=False, hovertemplate="<b>%{x} : </b>%{y}<extra></extra>")
        fig = self._update_layout(fig=fig)
        return fig
    ...
    def group_and_plot(
        ...
        if plot_type == "Pie":
            ...
            if show_labels:
                ...
                fig.add_trace(
                    eval(f"go.{plot_type}")(
                        x=list(grouped_df[x_var]), y=list(grouped_df[y_var]), marker_color=marker_color
                    )
                )
            fig.update_traces(
                hovertemplate=f"<b>{x_var} : </b>" + "%{x}<br>" + f"<b>{y_var} : </b>" + "%{y}<br>" + "<extra></extra>"
            )
    ...
    def plot_2_dimensional_data(
        ...
        if orientation == "h":
            temp = f"<b>{x_var} : </b>" + "%{y}<br>" + f"<b>{y_var} :</b>" + "%{x}<br>" + "<extra></extra>"
        else:
            temp = f"<b>{x_var} :</b>" + "%{x}<br>" + f"<b>{y_var} : </b>" + "%{y}<br>" + "<extra></extra>"

        fig.update_traces(hovertemplate=temp, marker_color=marker_color)
        ...
        if orientation == "h":
            ...
            fig.update_xaxes(title=x_var.title())
        fig.update_layout(
            title=dict(text=f"<b>{x_var.title()} Vs. {y_var.title()}</b></sup>", x=0.5),
        )
...
class DataTable(BasicPlots):
    ...
    def render_table(self, table_title: str = None, font_size: int = 12):
        ...
        fig.update_layout(
            height=h,
            margin=dict(l=0, r=0, t=0 if not table_title else 60, b=0),
            title=dict(text=f"<b>{table_title.upper()}</b>" if table_title else "<b>TABLE TITLE GOES HERE</b>", x=0.5),
        )
...
class CategoryPlots(BasicPlots):
    def bar_plot_by_categories_ordered_by_value(
        self,
        ...
        orderedDict = (
            df.sort_values(sort_cols, ascending=sort_ascending).groupby(groupByCol)[id_col].apply(list).to_dict()
        )
        ...
        for key, groups in orderedDict.items():
            for group in groups:
                value = df.loc[(df[id_col] == group) & (df[groupByCol] == key), y_var].iloc[0]
                if orientation == "v":
                    trace = go.Bar(
                        x=[key],
                        y=[value],
                        ...
                        legendgroup=group,
                        text_template="%{y}",
                        marker=dict(color=colors_dict.get(group)),
    ...
    def plot_by_categories(
        ...
        if df is not None:
            if not df.empty:
                if not all(col in df.columns for col in [id_col, x_var, y_var]):
...
class WordCloudGenerator:
    ...
    def generate_word_cloud(cls, words_list: list[str], word_counter: dict[str, int] = None) -> go.Figure:
        ...
        word_cloud = WordCloud(background_color="black", width=2160, height=600, colormap="Blues")
