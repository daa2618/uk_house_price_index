...
- Regenerate the static plot gallery under `src/ukhpi/images/`: `poetry run python -m ukhpi.plotting.generator`.
...
├── plotting/      # hpi_plots, categories, theme, save, generator
...
3. `ukhpi.plotting.hpi_plots :: HousePriceIndexPlots` wraps `HousePriceIndex` and exposes `plot_*` methods producing Plotly figures. `ukhpi.plotting.generator` discovers those methods by reflection (`dir(hpi_plots)` filtered by `startswith("plot")`) and saves each via `PlotSaver`.
...
- New plot methods should begin with `plot_` so `ukhpi.plotting.generator` picks them up automatically.
